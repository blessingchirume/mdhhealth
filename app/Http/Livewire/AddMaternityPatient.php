...
class AddMaternityPatient extends Component
{
    public $name;
    public $surname;
    public $dob;
    ...
    public function render()
    {
        ...
        return view('livewire.add-maternity-patient', compact('medicalAidProviders'));
    }
    ...
    public function queuePatient()
    {
        try {
            ...
            $data["patient_type"] = 'MaternityPatient';
            ...
            ->orWhere('item_description', 'LIKE', 'Provider Fee')->orWhere('item_description', 'LIKE', 'Initial Fee')->first();
            ...
            $this->resetForm();
            session()->flash('success', 'MATERNITY Patient added successfully!');

            // Close the modal after adding the patient
            $this->emit('patientAddedToQueue');
            $this->emit('closeModal');
        } catch (\Exception $e) {
            session()->flash('error', 'MATERNITY Patient could not be added successfully!<br/>' . $e->getMessage());
